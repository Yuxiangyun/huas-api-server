# 高校教务系统 API 服务

> 基于 Bun + Hono 的高性能教务系统中间件

## 📋 项目简介

这是一个为小程序提供的教务系统后端服务，通过代理学校教务系统（CAS + 正方/强智），解决了原系统存在的 Session 有效期短、并发限制、网络不稳定等问题。

### 核心特性

- 🔐 **持久化登录** - Token 与学校 Session 解绑，支持长期登录
- 🚀 **缓存优先** - 采用 SWR 策略，大幅提升响应速度
- 📱 **多端支持** - 同一用户可在多设备登录，数据互通
- 🧠 **智能刷新** - 差异化缓存策略，符合实际使用场景
- 🛡️ **安全防护** - 速率限制、SQL 注入防御、XSS 防护
- 📊 **数据统计** - 完善的用户统计和数据分析功能

## 🏗️ 技术栈

- **运行时**: [Bun](https://bun.sh/) - 极速 JavaScript 运行时
- **Web 框架**: [Hono](https://hono.dev/) - 轻量级高性能框架
- **语言**: TypeScript 5.x+
- **数据库**: SQLite (WAL 模式)
- **HTML 解析**: Cheerio
- **Cookie 管理**: tough-cookie

## 📦 快速开始

### 环境要求

- Bun >= 1.0
- Node.js >= 18 (可选，用于兼容)

### 安装依赖

```bash
bun install
```

### 开发模式

```bash
bun run dev
```

服务将在 `http://localhost:3000` 启动（开发模式）

### 生产模式

```bash
bun run start
```

## 🚀 脚本命令

### 开发与运行

```bash
bun run dev          # 开发模式（带热重载）
bun run start        # 生产模式
bun run build        # 构建项目
bun run lint         # 类型检查
```

### 测试

```bash
bun run test                # 运行所有测试
bun run test:unit           # 单元测试
bun run test:integration    # 集成测试
bun run test:e2e            # 端到端测试
bun run test:security       # 安全测试
bun run test:performance    # 性能测试
bun run test:coverage       # 测试覆盖率
bun run test:full           # 完整流程测试
```

### 数据库管理

```bash
bun run db:backup    # 备份数据库
bun run db:clean     # 清理过期数据
bun run db:stats     # 查看数据库统计
bun run db:vacuum    # 优化数据库
```

## 📚 文档

- [API 文档](./doc/API.md) - 完整的接口文档
- [架构设计](./doc/ARCHITECTURE.md) - 系统架构和设计理念
- [路由文档](./doc/ROUTES.md) - 路由系统详解
- [测试文档](./TEST.md) - 测试规范和覆盖

## 🌐 API 概览

### 认证模块

```
GET  /auth/captcha       # 获取验证码
POST /auth/login         # 用户登录
POST /auth/logout        # 退出登录
```

### 业务模块

```
GET /api/schedule        # 获取课表（支持缓存）
GET /api/ecard           # 获取一卡通（实时）
GET /api/user            # 获取用户信息（支持缓存）
```

### 系统模块

```
GET /system/health                  # 健康检查
GET /system/stats                   # 系统统计
GET /system/stats/users             # 用户统计
GET /system/stats/sessions          # 会话统计
GET /system/stats/cache             # 缓存统计
GET /system/stats/active-users      # 活跃用户排行
```

## 📂 项目结构

```
huas-api/
├── src/
│   ├── core/              # 核心层
│   │   ├── api/          # 外部 API 封装
│   │   ├── handlers/     # 处理器
│   │   ├── middleware/   # 中间件
│   │   ├── network/      # 网络层
│   │   └── utils/        # 工具类
│   ├── db/               # 数据层
│   │   ├── SessionRepo.ts    # 会话仓库
│   │   ├── CacheRepo.ts      # 缓存仓库
│   │   ├── UserRepo.ts       # 用户仓库
│   │   └── StatsRepo.ts      # 统计仓库
│   ├── parsers/          # 解析器层
│   ├── routes/           # 路由层
│   ├── services/         # 服务层
│   ├── test/             # 测试
│   └── server.ts         # 服务入口
├── scripts/              # 脚本工具
│   ├── db.sh            # 数据库管理
│   ├── deploy.sh        # 部署脚本
│   └── test.sh          # 测试脚本
├── doc/                 # 文档
└── deploy/              # 部署配置
```

## 🔧 配置

### 环境变量

创建 `.env` 文件（参考 `.env.example`）：

```bash
# 服务器配置
# 开发环境端口：3000
# 生产环境端口：12103
PORT=3000
NODE_ENV=production
CORS_ORIGINS=https://yourdomain.com

# 数据库配置
DB_PATH=/var/lib/huas-api/huas.sqlite

# 日志配置
LOG_LEVEL=INFO
LOG_FILE_PATH=/var/log/huas-api/app.log
LOG_ENABLE_CONSOLE=false
LOG_ENABLE_FILE=true

# 速率限制
LOGIN_RATE_LIMIT=10
CAPTCHA_RATE_LIMIT=20
API_RATE_LIMIT=60
```

## 🚢 部署

### 使用 systemd（推荐）

1. 首次安装

```bash
# 首次安装
sudo ./deploy/deploy.sh
```

2. 部署代码

```bash
sudo ./deploy/deploy.sh
```

3. 启动服务

```bash
# 查看服务状态
sudo systemctl status huas-api

# 查看日志
sudo journalctl -u huas-api -f
```

4. 查看状态

```bash
# 健康检查（生产环境）
curl http://localhost:12103/health

# 性能指标（生产环境）
curl http://localhost:12103/metrics
```

### 手动部署

```bash
# 1. 安装依赖
bun install --production

# 2. 配置环境变量
cp .env.example .env
vim .env

# 3. 启动服务
bun run start
```

## 📊 数据库

### 数据库设计

- **users** - 用户主表（学号、姓名、班级）
- **sessions** - 会话表（Token、Cookies、状态）
- **data_cache** - 缓存表（课表、一卡通、用户信息）

### 索引优化

- `idx_sessions_sid` - 加速多设备查询
- `idx_sessions_updated` - 加速会话清理
- `idx_cache_updated` - 加速缓存清理

### 维护命令

```bash
# 备份数据库
./scripts/db.sh backup

# 清理过期数据
./scripts/db.sh clean

# 查看统计
./scripts/db.sh stats

# 优化数据库
./scripts/db.sh vacuum
```

## 🔒 安全特性

- ✅ **Token 验证** - UUID v4 格式校验
- ✅ **速率限制** - IP 级别和用户级别限制
- ✅ **SQL 注入防御** - 使用 Prepared Statements
- ✅ **XSS 防护** - 输入验证和输出转义
- ✅ **敏感信息脱敏** - 日志中隐藏敏感数据
- ✅ **CORS 配置** - 生产环境白名单限制

## 📈 性能指标

| 指标 | 数值 |
|------|------|
| 缓存命中响应时间 | < 10ms |
| 网络请求响应时间 | 200-500ms |
| 并发支持 | 100+ QPS |
| 数据库大小 | ~50MB (1000 用户) |

## 🧪 测试

项目包含完整的测试套件：

- ✅ 单元测试 - 核心逻辑和工具函数
- ✅ 集成测试 - 数据库操作和服务层
- ✅ 端到端测试 - 完整业务流程
- ✅ 安全测试 - SQL 注入、XSS 防护
- ✅ 性能测试 - 缓存性能和并发

测试覆盖率 > 80%

## 🛠️ 故障排除

### 服务无法启动

```bash
# 查看服务状态
sudo systemctl status huas-api

# 查看日志
sudo journalctl -u huas-api -n 50

# 检查端口占用
# 开发环境
lsof -i :3000
# 生产环境
lsof -i :12103
```

### 数据库锁定

```bash
# 检查 WAL 模式
sqlite3 huas.sqlite "PRAGMA journal_mode;"

# 应返回 "wal"
```

### 日志文件过大

```bash
# 手动轮转日志
sudo logrotate -f /etc/logrotate.d/huas-api
```

## 🤝 贡献

欢迎贡献代码！请遵循以下规范：

1. Fork 项目
2. 创建特性分支 (`git checkout -b feature/AmazingFeature`)
3. 提交更改 (`git commit -m 'Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 开启 Pull Request

## 📝 开发规范

- 所有代码必须添加清晰的中文注释
- 遵循 TypeScript 严格模式
- 每次代码变更必须配套测试
- 提交信息使用中文，格式清晰

## 📄 许可证

本项目仅供学习交流使用

## 🔗 相关链接

- [Bun 官方文档](https://bun.sh/docs)
- [Hono 官方文档](https://hono.dev/)
- [SQLite 文档](https://www.sqlite.org/docs.html)

## 📮 联系方式

如有问题或建议，请提交 Issue

---

**注意**: 本项目需要配合学校教务系统使用，请确保拥有合法的访问权限。
